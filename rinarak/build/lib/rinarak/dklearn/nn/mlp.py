...
class FCBlock(nn.Module):
    def __init__(self,
                 ...
                 in_features,
                 out_features,
                 outermost_linear=False, activation = None):
        super().__init__()

        self.net = []

        if activation is None:
            activation = ["nn.CELU()" for i in range(num_hidden_layers + 2)]
        if isinstance(activation, str):
            activation = [activation for i in range(num_hidden_layers + 2)]
        
        self.net.append(FCLayer(in_features=in_features, out_features=hidden_ch, activation=activation[0]))

        for i in range(num_hidden_layers):
            self.net.append(FCLayer(in_features=hidden_ch, out_features=hidden_ch, activation=activation[i+1]))

        if outermost_linear:
            self.net.append(nn.Linear(in_features=hidden_ch, out_features=out_features))
        else:
            self.net.append(FCLayer(in_features=hidden_ch, out_features=out_features, activation = activation[-1]))
